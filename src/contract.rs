...
pub fn handle<S: Storage, A: Api, Q: Querier>(
    ...
    match msg {
        HandleMsg::SendToken {} => send_token(deps, env),
        HandleMsg::ResetAddr { address } => reset_addr(deps, env, address),
    }
}

pub fn send_token<S: Storage, A: Api, Q: Querier>(
    deps: &mut Extern<S, A, Q>,
    _env: Env,
) -> StdResult<HandleResponse> {
    if _env.message.sent_funds.is_empty() || "uusd" != _env.message.sent_funds[0].denom {
        return Err(StdError::generic_err(
            "Only supports the ust.",
        ));
    }
